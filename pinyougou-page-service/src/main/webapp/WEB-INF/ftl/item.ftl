    ...
    <script>
	   ...
	   var skuList=[    	    
	    	    <#list itemList as item>  	    	    	
		    		...
		    		"title":"${item.title!''}",
		    		"price":${item.price?c},		    		
		    		"spec": ${item.spec}//该属性不使用''直接解析为json对象	
		    		} ,     		
	    		</#list>
	   ];  
    </script>
...
<#-- goodsDesc中的图片字符串转换为对象 -->
<#assign imageList=goodsDesc.itemImages?eval>
<#-- 扩展属性 -->
<#assign customAttributeItems=goodsDesc.customAttributeItems?eval>
<#-- 规格列表 -->
<#assign specificationItems=goodsDesc.specificationItems?eval>{{sku	}}
<!--页面顶部 结束-->
				...
				<div class="fl preview-wrap">
					...
					<div class="zoom">
						...
						<div id="preview" class="spec-preview">
							<#if (imageList?size>0)>
								<span class="jqzoom"><img jqimg="${imageList[0].url}" src="${imageList[0].url}" width="400px" heigth="400px" /></span>
							</#if>
						</div>
						<!--下方的缩略图-->
